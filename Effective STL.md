...
有些 STL 容器提供了一些与算法同名的成员函数, 大多数情况下应该使用这些成员函数, 有两个原因:

- 成员函数往往速度快
- 成员函数通常与容器 (特别是关联容器) 结合得更加紧密

对于标准的关联容器, 选择成员函数而不选择对应的同名算法可以带来如下好处:

- 可以获得对数时间的性能, 而不是线性时间的性能
- 你可以使用等价性来确定两个值是否 "相同"
- 使用 map 和 multimap 的时候, 可以只考虑键的部分

对于 list 而言, 使用成员函数的原因是:

- remove, remove_if, unique, merge, sort 和 reverse 算法需要需要复制 list 容器中的对象, 而成员函数则无须任何对象副本, 它们操作的是指针
