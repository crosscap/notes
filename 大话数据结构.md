...
在 `str_index_KMP` 函数中, 将 `get_next` 替换为 `get_nextval` 即可
...
2. $ n > 1 $  时, 其余节点可分为 $ m (m > 0) $ 个互不相交的有限集 $ T_1, T_2, \cdots, T_n $ 其中每一个集合本身又是一棵树, 称为根的子树 (suntree)
